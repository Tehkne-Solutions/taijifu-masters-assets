...
def main()->int:
    for path in (SOURCE,P01,P02,P03,P04):
    ...
    if p04.get('source',{}).get('pixel_sha256')!=EXPECTED_PIXEL_SHA: return block('manifest_source_identity')
    ...
    if contract.get('guard_beats')!=['ready','brace','settle']: return block('guard_beats')
    if contract.get('dodge_beats')!=['anticipation','shift','peak_evade','return','recover']: return block('dodge_beats')
    unique=set(); total=0
    ...
    p02_count=sum(len(p02.get('frames',{}).get(m,[])) for m in ('jump_start','airborne','fall'))
    p03_count=len(p03.get('attack_light',[]))
    ...
    print('PRESET02_P04_CANDIDATE=PASS visual_review=pending')
